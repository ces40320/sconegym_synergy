...
from utils.sto2synergy_mat import generate_synergy_matrix
...
def batch_test_all_synergies(model_file,
                             ...
                             include_or_exclude=str(None)):
    # 1) 시너지 행렬 생성
    if include_or_exclude == 'include':
        print(f'Generating synergy matrix with INCLUDED muscles: {selected_muscles}')
        H, muscle_cols  = generate_synergy_matrix(data_path,
                                    n_components=n_components,
                                    include_muscles=selected_muscles)
    elif include_or_exclude == 'exclude':
        print(f'Generating synergy matrix with EXCLUDED muscles: {selected_muscles}')
        H, muscle_cols  = generate_synergy_matrix(data_path,
                                    n_components=n_components,
                                    exclude_muscles=selected_muscles)
    else:
        print(f'Generating synergy matrix with ALL muscles')
        H, muscle_cols  = generate_synergy_matrix(data_path,
                                    n_components=n_components)
...
if __name__ == '__main__':
    MODEL_FILE = r'sconegym\data-v1\H2190_SynergyTest_torsoRL.scone'
    DATA_PATH  = r'E:\Dropbox\walk_rl\Data\mocap\MocoInv\H2190\BeforeTendonCompliance_R'
    # N_SYNERGY  = list(range(4,13))  # 테스트할 시너지 개수 범위 (4~12)
    N_SYNERGY  = list(range(10,11))  # 테스트할 시너지 개수 범위 (4~12)
    SELECTED   = ['rect_abd_r', 'ext_obl_r', 'int_obl_r', 'quad_lumb_r', 'erec_sp_r']
    include_or_exclude = 'include'  # 'include' or 'exclude'
